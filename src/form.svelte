<script>
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (
        extensionConfiguration?.closeAddBookmarkWindowOnSave === true &&
        extensionConfiguration?.closeAddBookmarkWindowOnSaveMs >= 0
      ) {
        window.setTimeout(() => {
          window.close()
  ...
  function handleLinks() {
    openLinks(configuration.baseUrl)
  }
...
<div class="title-row">
  ...
  <div>
    <a href="#" on:click|preventDefault={handleLinks}>Links</a>
    &nbsp;|&nbsp;
...
<form class="form" on:submit|preventDefault={handleSubmit}>
  <div class="form-group">
    ...
    {#if bookmarkExists}
      <div class="form-input-hint text-warning">
        This URL is already bookmarked. The form has been prefilled from the
        existing bookmark, and saving the form will update the existing
        bookmark.
      </div>
    {/if}
  </div>
  ...
  <div class="form-group">
    ...
    {#if editNotes}
      ...
      <textarea class="form-input" id="input-notes" rows="5" bind:value={notes}
      ></textarea>
    {/if}
  </div>
    ...
    {#if saveState !== "success"}
      <div class="button-row">
        <button
          ...
          class:loading={saveState === "loading"}>Save</button
        >
      </div>
    {/if}
  </div>
